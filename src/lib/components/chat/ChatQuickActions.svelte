<script lang="ts">
  import { addUserMessage, setTyping, addAssistantMessage } from '$lib/stores/chat';
  ...
  function handleQuickAction(message: string) {
    ...
    setTimeout(() => {
      ...
      const response =
        responses[message] || 'Gracias por tu mensaje. José suele responder en menos de 24 horas.';
      addAssistantMessage(response);
    }, 1500);
  }
</script>

<div class="border-t border-border-primary px-4 py-2">
  <div class="flex gap-2 overflow-x-auto pb-2">
    {#each quickActions as action}
      <button
        onclick={() => handleQuickAction(action.message)}
        class="whitespace-nowrap rounded-full bg-bg-tertiary px-3 py-1.5 text-xs text-text-secondary transition-colors hover:bg-bg-secondary"
      >
        {action.label}
      </button>
    {/each}
